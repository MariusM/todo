...
export default function App() {
  const { todos, isLoading, addTodo, updateTodo } = useOptimisticTodos()
  ...
  return (
    <div className="min-h-screen bg-surface-secondary">
      <main className="mx-auto max-w-[var(--max-content-width)] p-4">
        ...
        <div className="mt-4">
          <TaskList todos={todos} isLoading={isLoading} onToggle={(id, completed) => updateTodo(id, { completed })} onEdit={(id, text) => updateTodo(id, { text })} />
        </div>
      </main>
    </div>
  )
